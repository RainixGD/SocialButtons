...
public:
	...
	__declspec(dllexport) static void init(CCNode* self) {
		...
		if (once) {
			int n = 2048; char* buffer = new char[n + 1]; buffer[n] = 0;
			file.read(buffer, n);
			...
			cJSON_ArrayForEach(btn, buttons) {
				cJSON* isBtn = cJSON_GetObjectItemCaseSensitive(btn, "isBtn");
				cJSON* texture = cJSON_GetObjectItemCaseSensitive(btn, "texture");
				cJSON* link = cJSON_GetObjectItemCaseSensitive(btn, "link");

				BtnsOptions.isActive[btnCounter] = isBtn->valueint;
				BtnsOptions.textures[btnCounter] = texture->valuestring;
				BtnsOptions.links[btnCounter] = link->valuestring;
				btnCounter++;
			}
			once = 0;
		}
		...
		for (int i = 0; i < childrenCount; i++) {
			...
			if ((obj->getPosition().x >= 50 && obj->getPosition().x <= 51) && (obj->getPosition().y >= 24 && obj->getPosition().y <= 25)) {
				...
				for (int i = 1; i < 7; i++) {
					if (!BtnsOptions.isActive[i])
						continue;

					auto btnSprite = CCSprite::create(BtnsOptions.textures[i].c_str());
					if (btnSprite == NULL)
						btnSprite = CCSprite::createWithSpriteFrameName("GJ_deleteIcon_001.png");
					btnSprite->setScale(0.8);
					auto btn = CCMenuItemSpriteExtra::create(btnSprite, obj, menu_selector(SocialBtns::onClick));

					if (i < 4) upMenu->addChild(btn, 5, 100 + i);
					else downMenu->addChild(btn, 5, 100 + i);
				}
				upMenu->alignItemsHorizontallyWithPadding(5);
				downMenu->alignItemsHorizontallyWithPadding(5);
			}
